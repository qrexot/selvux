...
const supabaseClient = window.supabase.createClient(supaUrl, supaKey);
...
function initAuth() {
  ...
  btn.onclick = async () => {
    ...
    if (!/^[a-zA-Z0-9_]{3,15}$/.test(val)) {
      err.textContent = 'Только a-z, 0-9, _, 3-15 символов.';
      return;
    }
    ...
    // Check if exists
    const { data, error } = await supabaseClient.from('users').select('nickname').eq('nickname', val).single();
    ...
    if (data) {
      ...
      // Create
      const { error: insErr } = await supabaseClient.from('users').insert([{ nickname: val, time_spent: timeSpent }]);
      if (insErr) {
        err.textContent = 'Ошибка: ' + insErr.message;
        btn.disabled = false;
        btn.textContent = 'Войти';
      } else {
        myNickname = val;
...
async function updateSupabaseTime() {
  ...
  await supabaseClient.from('users').update({ time_spent: timeSpent }).eq('nickname', myNickname);
}
...
async function renderLeaderboard() {
  ...
  const { data, error } = await supabaseClient.from('users').select('*').order('time_spent', { ascending: false }).limit(50);
  ...
  data.forEach((user, idx) => {
    ...
    // Add staggered delay based on index for smooth entry
    el.style.animationDelay = `${idx * 0.05}s`;
    
    el.innerHTML = `
      <div class="lb-rank">#${idx + 1}</div>
      <div class="lb-name">${user.nickname}</div>
      <div class="lb-time">${formatTime(user.time_spent)}</div>
    `;
